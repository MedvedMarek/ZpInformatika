...
class Stahovanie_OHLC:
  ...
  def stiahni_data_pre_OHLC(self):
    ...
    for i in self.datumy:
      # os.mkdir(self.path+i.replace('-','_')) nevytvaram zlozku, lebo len doplnam k existujucim burzam
      for j in self.burzy:
...
for i in ['5m','10m','15m','20m','30m','1h','2h','4h']:
        #for j in ['trades','midpoint','bid','ask']:
        for j in ['trades']:
                Stahovanie_OHLC(i,j,1)
