...
class Bot:
  ...
  def __init__(self):
    ...
    self.x = np.arange(7, dtype=float)
    self.y = np.arange(7, dtype=float)
  ...
  def on_bar_update(self, reqId, time, open_, high, low, close, volume, wap, count):
    ...
    print(open_, high, low, close, volume, wap, count)
    self.y[0], self.y[1], self.y[2], self.y[3], self.y[4] self.y[5], self.y[6] = open_, high, low, close,volume, wap, count
