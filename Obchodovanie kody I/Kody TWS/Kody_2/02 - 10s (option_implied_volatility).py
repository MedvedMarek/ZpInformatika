...
class Stahovanie_OHLC:
  ...
  def __init__(self):
    ...
    self.burzy  = ['SMART']
    self.path   = '/media/marek/zaloha/Data/OHLC_TWS/AAPL/10s/option_implied_volatility/' #----------------------------------------------------
  ...
  def stiahni_data_pre_OHLC(self):
    ...
    for i in self.datumy:
      ...
      for j in self.burzy:
        ...
        datum = i.replace('-','')
        time.sleep(1)
        app.reqHistoricalData(1,amd, datum+'-21:00:00', '23400 S', '10 secs', 'OPTION_IMPLIED_VOLATILITY', 1, 1, False, []) #-------------------------
        time.sleep(1)
