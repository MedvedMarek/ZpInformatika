...
def najdi_neuplne_stiahnute_data():
  ...
  """
  path   = '/media/marek/zaloha/Data/OHLC_TWS/AAPL/5s/midpoint/' #------------------------------------------------------
  zlozky = os.listdir(path)
  x = []
  for i in zlozky:
    for j in os.listdir(path+i):
      if os.path.getsize(path+i+'/'+j) < 2000000: #--------------------------------------------------------------------
...
class Stahovanie_OHLC:
  ...
  def __init__(self):
    ...
    self.path   = '/media/marek/zaloha/Data/OHLC_TWS/AAPL/5s/midpoint/' #---------------------------------------------
  ...
  def stiahni_data_pre_OHLC(self):
    ...
    for i,j in self.datum_a_burza:
      ...
      datum = i.replace('-','')
      app.reqHistoricalData(1,amd, datum+'-21:00:00', '23400 S', '5 secs', 'MIDPOINT', 1, 1, False, []) #---------------
      time.sleep(2)
