...
# Rozdiel oproti povodnemu kodu je ten, ze predikciu doplnam pre vsetky intervaly 1,2,5,10m.
...
class Priprava_dat:
  ...
  def __dopln_min_max_predch_dna(self):
    ...
    for i in self.datumy[1:]:
      ...
      _close= data['c1'][index[0]-1]
      _min  = data['c1'][index1].min()
      _max  = data['c1'][index1].max()
      
      data.loc[index,'close'] = data.loc[index,'o1'] - _close
      data.loc[index,'min']   = data.loc[index,'o1'] - _min
      data.loc[index,'max']   = data.loc[index,'o1'] - _max
  ...
  def __dopln_predikciu(self):
    ...
    data.insert(data.shape[1],'co5',0)
    data.insert(data.shape[1],'co10',0)
    data.co1  = np.append(data['o1'][1:].to_numpy(),0)  # pripajam ku koncu nulu
    data.co2  = np.append(data['o2'][1:].to_numpy(),0)  # pripajam ku koncu nulu
    data.co5  = np.append(data['o5'][1:].to_numpy(),0)  # pripajam ku koncu nulu
    data.co10 = np.append(data['o10'][1:].to_numpy(),0) # pripajam ku koncu nulu
    return data
  ...
  def __uprav_ohlc(self):
    ...
    for i in self.datumy:
      ...
      data.loc[index,'co1'] = data.loc[index,'co1'] - _open
      data.loc[index[:1],'co1'] = 0
      data.loc[index[-1],'co1'] = data.loc[index[-2],'co1']
      
      data.loc[index,'co2'] = data.loc[index,'co2'] - _open
      data.loc[index[:2],'co2'] = 0
      data.loc[index[-1],'co2'] = data.loc[index[-2],'co2']
      
      data.loc[index,'co5'] = data.loc[index,'co5'] - _open
      data.loc[index[:5],'co5'] = 0
      data.loc[index[-1],'co5'] = data.loc[index[-2],'co5']
      
      data.loc[index,'co10'] = data.loc[index,'co10'] - _open
      data.loc[index[:10],'co10'] = 0
      data.loc[index[-1],'co10'] = data.loc[index[-2],'co10']
...
priprava = Priprava_dat()
data = priprava.dodaj_data()
data.to_csv('/home/marek/Data/AMD/1m/1m_vsetko.csv', index=False)
