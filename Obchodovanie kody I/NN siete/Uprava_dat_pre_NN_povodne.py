...
class Priprava_dat:
  ...
  def __nacitaj_a_spoj_burzy_za_jeden_den(self,den):
    ...
    """
    data = np.load(self.path+den+'/AMD_AMEX.npy')
  ...
  def __normalizuj_data(self):
    ...
    # >>> BEXv: 15
    # ...
    volume = {'AMEv':13,'ARCv':170,'BATv':100,'BEXv':15,'BYXv':20,'CHXv':11,'DRCv':260,'EDGv':25,
              'IEXv':60,'MEMv':70,'NYSv':55,'PSXv':20,'SMAv':1300,'PEAv':17,'ISLv':500}
    for i in volume:
    ...
    ww     = {'AMEw':13,'ARCw':170,'BATw':100,'BEXw':15,'BYXw':20,'CHXw':11,'DRCw':260,'EDGw':25,
              'IEXw':60,'MEMw':70,'NYSw':55,'PSXw':20,'SMAw':1300,'PEAw':17,'ISLw':500}
    for i in ww:
      data[i] = data[i]/ww[i]
      data[i] = data[i].apply(lambda x: x.__round__(6))
