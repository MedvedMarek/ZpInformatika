...
class Bot:
  ...
  def __init__(self):
    ...
    self.x8 = np.arange(8, dtype=float)
    self.x9 = np.arange(8, dtype=float)
    self.x10 = np.arange(8, dtype=float)
    self.x11 = np.arange(8, dtype=float)
    self.x12 = np.arange(8, dtype=float)
    self.x13 = np.arange(8, dtype=float)
    self.x14 = np.arange(8, dtype=float)
    self.x15 = np.arange(8, dtype=float)
    
    self.y = np.arange(8, dtype=float)
    
    self. data = {'1':self.x1, '2':self.x2, '3':self.x3, '4':self.x4, '5':self.x5, '6':self.x6, '7':self.x7, 
                  '8':self.x8, '9':self.x9, '10':self.x10,'11':self.x11, '12':self.x12, '13':self.x13, '14':self.x14, 
                  '15':self.x15}
  ...
  def on_bar_update(self, reqId, time, open_, high, low, close, volume, wap, count):
    ...
    self.data['{}'.format(reqId)] = np.vstack([self.data['{}'.format(reqId)], self.y])
    if time % 5 == 0:
      np.save("/home/marek/wx/"+akcia+'_'+self.nazov_burzy['{}'.format(reqId)], self.data['{}'.format(reqId)])
