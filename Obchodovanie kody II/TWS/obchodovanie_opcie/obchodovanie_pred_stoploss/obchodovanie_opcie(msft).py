...
vlakno = 5  # vlakno je potrebne mat rozne  pri viacerych otvorenych suboroch tws
burza = 'MSFT'
...
class IBApi(EWrapper, EClient):
    ...
    def orderStatus(self, orderId, status, filled, remaining, avgFillPrice, permId, parentId, lastFillPrice, clientId, whyHeld, mktCapPrice):
        ...
        if status == 'Filled':
            # toto je skumanie, ci sa nachadza v pocitadle id pre limitnu objednavku. Ak ako, tak sa z pocitadla id odstrani, lebo objednavka
        ...
        if status in ['Inactive', 'Cancelled']:
            ...
            # cancelled - toto znamena, ze objednavka bola oficialne zrusena. To je asi zrusenie cez konzolu.
    ...
    def predaj_opcie(self, expiracia, strike, right, quantity):
        ...
        order.totalQuantity = quantity
        order.orderType = "LMT"
        order.lmtPrice = self.execution_price + 0.05  # execution_price je pocitane automaticky. hodnota 0.15 je o kolko sa ma navysit cena aby sa predala opcia.
        order.eTradeOnly = ''  # Toto je iba osetrenie vynimky. Vyhadzovalo to a podla netu to staci takto osetrit.
        order.firmQuoteOnly = ''  # Toto je to iste ako v predchadzajucom riadku.
        nove_id = self.nextOrderId()  # toto sluzi v pocitadle pre pocet otvorenych objednavok
        self.pocitadlo_pre_opcie.append(nove_id)  # tu sa uchovavaju id pre limitne objednavky. Podla toho sa bude urcovat, ze ak bude viac objednavok ako hranica, tak sa objednavka nezrealizuje.
        self.placeOrder(nove_id, contract, order)  # Poslanie objednávky
...
strike_ceny = [370, 372.5, 375, 377.5, 380, 382.5, 385, 387.5, 390, 392.5, 395, 397.5, 400, 402.5, 405, 407.5, 410, 412.5, 415, 417.5, 420]
...
def bot_opcie():
    ...
    while True:
        ...
        if (pocitadlo > 0) and (len(bot.ib.pocitadlo_pre_opcie) < 5):  # na nakup musia byt nejake spustace a nesmia byt otvorenych viac limitnych objednavok ako 4.
            ...
            #if bot.ib.orderStatus:  # toto znamena, ze objednavka bola vyplnena a nenastala ziadna komplikacia.
                # bot.ib.predaj_opcie(expiracia_opcie, strike, cp, pocitadlo)
