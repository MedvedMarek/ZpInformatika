...
vlakno = 109  # vlakno je potrebne mat rozne  pri viacerych otvorenych suboroch tws <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
burza = 'SPY' # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
class IBApi(EWrapper, EClient):
    def __init__(self):
        ...
        self.kluc_limit_hodnota_stoploss = {} # Sem sa ukladaju id pre par limit a stoploss. Sluzi na kontrolu a odstranovanie stoposs objednavky
        self.kluc_stoploss_hodnota_limit = {} # Sem sa ukladaju ic pre par stoploss a limit. Je to to iste ako slovnik hore.
    ...
    def orderStatus(self, orderId, status, filled, remaining, avgFillPrice, permId, parentId, lastFillPrice, clientId, whyHeld, mktCapPrice):
        ...
        print(f"Order: {orderId}, status: {status}, filled: {filled}, remaining: {remaining}, avgFillPrice: {avgFillPrice}")
        ...
        if status == 'Filled':
            ...
            if orderId in self.kluc_limit_hodnota_stoploss.keys() or orderId in self.kluc_stoploss_hodnota_limit.keys():
                if orderId in self.kluc_limit_hodnota_stoploss.keys():
                    #self.pocitadlo_pre_opcie.remove(orderId)
                    #self.pocitadlo_pre_opcie.remove(self.kluc_limit_hodnota_stoploss[orderId])
                    self.cancelOrder(self.kluc_limit_hodnota_stoploss[orderId])

                if orderId in self.kluc_stoploss_hodnota_limit.keys():
                    #self.pocitadlo_pre_opcie.remove(orderId)
                    #self.pocitadlo_pre_opcie.remove(self.kluc_stoploss_hodnota_limit[orderId])
                    pass
            else:
    ...
    def predaj_opcie(self, expiracia, strike, right, quantity):
        ...
        limit_order.lmtPrice = self.execution_price + 0.10  # execution_price je pocitane automaticky. hodnota 0.15 je navysenie. <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        stop_loss_order.auxPrice = self.execution_price - 0.25 # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        self.kluc_limit_hodnota_stoploss[limit_order_id] = stop_loss_order_id
        self.kluc_stoploss_hodnota_limit[stop_loss_order_id] = limit_order_id
...
cas = np.repeat(1716997765, 11)
strategia = f'{burza.lower()}_okolie_20' # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
strategia_opcie = nacitaj_strategiu(0.97, strategia)
strike_ceny = [530, 531, 532, 532.5, 533, 534, 535, 536, 537, 537.5, 538, 539, 540, 541, 542, 542.5, 543, 544, 545, 546, 547, 547.5, 548, 549, 550, 551, 552, 552.5, 553, 554, 555, 556, 557, 558, 559, 560, 561, 562, 563, 564, 565] # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
