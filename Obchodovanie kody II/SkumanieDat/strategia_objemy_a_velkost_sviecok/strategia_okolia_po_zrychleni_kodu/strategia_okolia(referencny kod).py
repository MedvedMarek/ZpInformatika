...
class Strategia_12(Base):
    ...
    """
    __tablename__ = nazov_tabulky(12)
...
class Strategia_14(Base):
    ...
    pr: float, prevdepodobnost spravneho rozhodnutia, ak budu aj v dalsom obchode rovnake parametre
    """
    __tablename__ = nazov_tabulky(14)
    id     = Column(Integer, primary_key=True, autoincrement=True)
    burza  = Column(String)
    ob_min = Column(Integer)
    ob_max = Column(Integer)
...
class Strategia_16(Base):
    ...
    pr: float, prevdepodobnost spravneho rozhodnutia, ak budu aj v dalsom obchode rovnake parametre
    """
    __tablename__ = nazov_tabulky(16)
    id     = Column(Integer, primary_key=True, autoincrement=True)
    burza  = Column(String)
    ob_min = Column(Integer)
    ob_max = Column(Integer)
...
class Strategia_18(Base):
    ...
    pr: float, prevdepodobnost spravneho rozhodnutia, ak budu aj v dalsom obchode rovnake parametre
    """
    __tablename__ = nazov_tabulky(18)
    id     = Column(Integer, primary_key=True, autoincrement=True)
    burza  = Column(String)
    ob_min = Column(Integer)
    ob_max = Column(Integer)
...
class Strategia_20(Base):
    ...
    pr: float, prevdepodobnost spravneho rozhodnutia, ak budu aj v dalsom obchode rovnake parametre
    """
    __tablename__ = nazov_tabulky(20)
    id     = Column(Integer, primary_key=True, autoincrement=True)
    burza  = Column(String)
    ob_min = Column(Integer)
    ob_max = Column(Integer)
...
def zluc_data(burza) -> pd.DataFrame:
    ...
    for i in datum:
        ...
        zlucene_data[i] = data[['open', 'close', burza]]

    return zlucene_data


# Alokovanie pandas dat na to, aby sa to nemusel stale alokovat nanovo.
smart_df = pd.DataFrame(np.zeros((4680, 3)), columns=['time', 'open', 'close']).astype({'time': 'int', 'open': 'float','close': 'float'})
...
def vypocitaj_strategiu(datum, objem, okolie, velkost_sviecky, burza) -> pd.DataFrame:
    ...
    data['sviecka'] = data['close'] - data['open'] # > 0 je rastuca sviecka, < 0 je klesajuca sviecka 
    data['velkost_sv'] = abs(data['open'] - data['close'])    
...
velkost_sviecky = [[0.01, 0.02], [0.02, 0.03], [0.03, 0.04], [0.04, 0.05], [0.05, 0.06], [0.06, 0.07], [0.07, 0.08],
...
for m in burzy:
    for i in objem:
        for l in velkost_sviecky:
            ...
            for k in datum:
                data = vypocitaj_strategiu(k, i, 0.12, l, m)
            ...
            zapis = Strategia_12(burza=m, ob_min=i[0], ob_max=i[1], sv_min=l[0], sv_max=l[1], true=true, false=false, suma=true+false, pr=pravdep)
            session.add(zapis)
            session.commit()
...
for m in burzy:
    for i in objem:
        for l in velkost_sviecky:
            ...
            for k in datum:
                data = vypocitaj_strategiu(k, i, 0.14, l, m)
                velkost = data.shape[0]
                suma = data['trigger'].sum()
                true = true + suma
                false = false + (velkost - suma)
            ...
            pravdep = true/(true+false) if true+false != 0 else 0    
            zapis = Strategia_14(burza=m, ob_min=i[0], ob_max=i[1], sv_min=l[0], sv_max=l[1], true=true, false=false, suma=true+false, pr=pravdep)
            session.add(zapis)
            session.commit()
...
for m in burzy:
    for i in objem:
        for l in velkost_sviecky:
            ...
            for k in datum:
                data = vypocitaj_strategiu(k, i, 0.16, l, m)
                velkost = data.shape[0]
                suma = data['trigger'].sum()
                true = true + suma
                false = false + (velkost - suma)
            ...
            pravdep = true/(true+false) if true+false != 0 else 0    
            zapis = Strategia_16(burza=m, ob_min=i[0], ob_max=i[1], sv_min=l[0], sv_max=l[1], true=true, false=false, suma=true+false, pr=pravdep)
            session.add(zapis)
            session.commit()
...
for m in burzy:
    for i in objem:
        for l in velkost_sviecky:
            ...
            for k in datum:
                data = vypocitaj_strategiu(k, i, 0.18, l, m)
                velkost = data.shape[0]
                suma = data['trigger'].sum()
                true = true + suma
                false = false + (velkost - suma)
            ...
            pravdep = true/(true+false) if true+false != 0 else 0    
            zapis = Strategia_18(burza=m, ob_min=i[0], ob_max=i[1], sv_min=l[0], sv_max=l[1], true=true, false=false, suma=true+false, pr=pravdep)
            session.add(zapis)
            session.commit()
...
for m in burzy:
    for i in objem:
        for l in velkost_sviecky:
            ...
            for k in datum:
                data = vypocitaj_strategiu(k, i, 0.20, l, m)
                velkost = data.shape[0]
                suma = data['trigger'].sum()
                true = true + suma
                false = false + (velkost - suma)
            ...
            pravdep = true/(true+false) if true+false != 0 else 0    
            zapis = Strategia_20(burza=m, ob_min=i[0], ob_max=i[1], sv_min=l[0], sv_max=l[1], true=true, false=false, suma=true+false, pr=pravdep)
            session.add(zapis)
            session.commit()
