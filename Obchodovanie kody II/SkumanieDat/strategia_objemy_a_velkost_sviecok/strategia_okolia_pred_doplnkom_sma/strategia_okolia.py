...
def vypocitaj_strategiu(datum, objem, okolie, velkost_sviecky, burza) -> pd.DataFrame:
    ...
    data = data[(data['sviecka'] >= velkost_sviecky[0]) & (data['sviecka'] < velkost_sviecky[1])]
    ...
    for m in burzy:
        for i in objem:
            for l in velkost_sviecky:
                ...
                for k in datum:
                    data = vypocitaj_strategiu(k, i, oko, l, m)
                ...
                zapis = triedy[n](burza=m, ob_min=i[0], ob_max=i[1], sv_min=l[0], sv_max=l[1], true=true, false=false, suma=true+false, pr=pravdep)
                session.add(zapis)
                session.commit()
            
                print(f'burza: {m:<8}  objem: [{i[0]:<3} {i[1]:<3}]  sviecka: [{l[0]:<5} {l[1]:<5}]  ture: {true:<5}  false: {false:<5}  pr: {pravdep:.4f}')
            print('')
