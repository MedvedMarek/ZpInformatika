...
def nacitaj_data(datum, interval) -> Dict[str, np.ndarray]:
    ...
    # nezapocitanie prvych a poslednych dat. Lebo vykresluju neumerne velke kruhy
    data = {x:data[x][2:-2] for x in burzy_male}
...
#datum = '2024_01_12'
#data = nacitaj_data(datum, 30)
data = nacitaj_data(args.datum, args.interval)


@app.route('/')
def index():
    return render_template('skumanieDat.html')
...
def data_route():
    response_data = {
        ...
        **{f'{j}_v': (data[j]['volume']/2).tolist() for j in burzy_male2}
    }
