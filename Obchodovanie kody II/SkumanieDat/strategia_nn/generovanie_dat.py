...
def generuj_data(akcia, pravdepodobnost, okolie, stop_loss, burzy, lookback):
    ...
    vygenerovane_data = np.zeros(shape=(pocet_riadkov, lookback, 4+len(burzy))) # styry znamena [sviecka, knot_up, knot_down, open_close]
    # vygenerovane_data = np.zeros(shape=(pocet_riadkov, 360, 3+len(burzy))) # tri znamena [sviecka, knot_up, knot_down]

    vygenerovane_buy = []
    pocitadlo = 0
    final_column = ['sviecka', 'knot_up', 'knot_down', 'open_close']

    for x in burzy:
        final_column.append(x)

    # final_column = ['sviecka', 'knot_up', 'knot_down'] + burzy

    for datum in datumy:
        if len(ind[datum]) > 0:
            for index in ind[datum]:
                try:
                    # vygenerovane_data[pocitadlo] = data[datum][index-359:index+1][final_column].to_numpy()
...
# data = generuj_data('aapl', 0.95, 12, 30, ['arca', 'smart'], 700)
