...
#
#   princip:   su vygenerovane data z uniformneho rozdelenia
#              na intervale pre definicny obor elipsy. Tento
#              interval je rozdeleny na polovicu, aby mohla
#              byt pouzita funkcia pre spodnu a hornu hranu
#
# parametre:
#           B: parameter elipsy
#           C: parameter elipsy
...
#
genDataOnElipse = function(B,C,D,F,G,velkost,intForX){
  
  interval = intForX
  polovica = as.integer(velkost/2)

  mi_1 = runif(polovica, interval[1,1], interval[2,1])
  mi_2 = runif(velkost - polovica, interval[1,1], interval[2,1])
  mi   = c(mi_1, mi_2)
  ni_1 = getElipse_m(mi_1,B,C,D,F,G)
  ni_2 = getElipse_p(mi_2,B,C,D,F,G)
  ni   = c(ni_1, ni_2)
  mini = cbind(mi,ni)
  data = data.frame(mini)
  
  return(data)
}
# interval = getXIntervalForElipse(3,-2,-5,-4,10)
# genDataOnElipse(3,-2,-5,-4,10, 100,interval)
#
#           mi        ni
# 1   3.468778 0.4961949
# 2   6.998497 1.9987482
# 3   2.393602 0.5205603
# 4   7.617272 2.6598118




#
# vygenerovanie dat na elipse, ale toto je pre rovnako rozdelene casti
# na definicnom obore. 
#
#   princip:   su vygenerovane data z rovnakeho rozdelenia
#              intervalu pre definicny obor elipsy. Tento
#              interval je rozdeleny potom na polovicu, aby mohla
#              byt pouzita funkcia pre spodnu a hornu hranu
#
# parametre:
#           B: parameter elipsy
...
#    vystup: - dataframe kde su vygenerovane data na elipse
#
genDataOnElipseSegmenty = function(B,C,D,F,G,velkost,intForX){
  
  interval = intForX
  polovica = as.integer(velkost/2)
  
  mi_1 = seq(from = interval[1,1], to = interval[2,1], length.out = polovica)
  mi_2 = seq(from = interval[1,1], to = interval[2,1], length.out = velkost - polovica)
  mi   = c(mi_1, mi_2)
  ni_1 = getElipse_m(mi_1,B,C,D,F,G)
  ni_2 = getElipse_p(mi_2,B,C,D,F,G)
  ni   = c(ni_1, ni_2)
...
# interval = getXIntervalForElipse(3,-2,-5,-4,10)
# genDataOnElipse(3,-2,-5,-4,10, 101,interval)
#
#           mi        ni
# 1   3.468778 0.4961949
# 2   6.998497 1.9987482
...
#
generujData = function(param, velkost, vypis = F){
  ...
  dataOnElipse = genDataOnElipse(B,C,D,F,G,velkost, intForX)
...
S_0 = function(A0, Q11, W1, W2, W3){
  ...
  X = mat.mult(mat.mult(t(A0),Q11),A0)
  
  s = matrix(0,3,3)
  
  # Zjednotenie rovnakych vypoctov do jednej matice
  X_W1_X = mat.mult(mat.mult(X, W1), X)
  X_W2_X = mat.mult(mat.mult(X, W2), X)
  X_W3_X = mat.mult(mat.mult(X, W3), X)
...
######## 4. uprava funkcii pre testy ###########################


# nahodne generovanie
genDataOnElipse1 = function(param, velkost){
  ...
  interval = getXIntervalForElipse(B,C,D,F,G)
  polovica = as.integer(velkost/2)
...
aproxDataTesty = function(par, parApr, velkost, iter, plot = F, vypis = F){
  
  # pocitanie iteracii pre legendu do grafov
  iteracia = 1
  
  # matica na testovanie
  testIteracia = matrix(data = NA, ncol = 8, nrow = iter)
  colnames(testIteracia) = c("B","C","D","F","G","ro","sigma","delta")
  ...
  if(iter > 0){
    ...
    param = param_nove(paramVstup, odhad, vypis)
    testIteracia[1,] = round(c(param[[4]][1,1],
                               param[[4]][2,1],
                               param[[4]][3,1],
                               param[[4]][4,1],
                               param[[4]][5,1],
    ...
    if(iter > 1){
      for (i in 1:(iter-1)) {
        ...
        param = param_nove(param, odhad, vypis)
        testIteracia[iteracia,] = round(c( param[[4]][1,1],
                                           param[[4]][2,1],
                                           param[[4]][3,1],
                                           param[[4]][4,1],
                                           param[[4]][5,1],
  ...
  invisible(list(vystup, testIteracia))
}
